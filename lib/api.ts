...
export async function deleteOutlet(storeId: number) {
  try {
    const session = await getServerSession(authOptions);
    if (!session) return { error: "Unauthorized" };
    const user = (session as any).user;

    // 1. Verify ownership and CORPORATE plan
    const store = await prisma.store.findUnique({
      where: { id: storeId },
      select: { ownerId: true, subscriptionPlan: true, slug: true }
    });
    ...
    if (store.ownerId !== parseInt(user.id)) return { error: "Unauthorized: You do not own this outlet" };
    
    // Check if the user has a CORPORATE plan somewhere (to ensure they are a corporate user)
    const userStores = await prisma.store.findMany({
      where: { ownerId: parseInt(user.id) },
      select: { subscriptionPlan: true }
    });
    const isCorporate = userStores.some(s => s.subscriptionPlan === "CORPORATE");
    if (!isCorporate) return { error: "Unauthorized: Only Corporate accounts can delete outlets from the dashboard" };

    // 2. Perform cascading delete (similar to super-admin logic but for the merchant)
