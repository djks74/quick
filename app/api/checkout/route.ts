...
export async function POST(req: NextRequest) {
  try {
    ...
    const isFast = req.headers.get('x-fast-checkout') === '1' || Boolean((body as any)?.fast);
    ...
    if (isFast) {
      // Defer notifications and shipping to improve payment opening speed
      void notifyAndShipping();
    } else {
      await notifyAndShipping();
    }
